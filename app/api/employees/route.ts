...
// // 2. POST: Admin creating a new INTERNAL Employee
...
//     const { name, email, password, role, department, position, phone, monthlyTarget } = body;
...
//       status: "active", department, position, monthlyTarget: Number(monthlyTarget) || 0,
//     });
...
// // 👇 NEW: 3. PUT: Update Employee (Edit Details or Suspend Status)
...
//       const body = await request.json();
//       const { id, name, email, role, department, position, phone, monthlyTarget, status } = body;
...
//       if (status) updateData.status = status; // Used for Suspending/Activating
