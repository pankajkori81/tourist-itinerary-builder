...
import { 
  ...
  Star, Moon, Car, Plane, Train, Ship, ArrowRight, ArrowLeft,
  Ban, CheckCircle2, PlusSquare, Briefcase  , Send
} from 'lucide-react';
...
export default function DaywisePage() {
  const router = useRouter();
  const { user } = useUser();
  const { itineraryData, updateItineraryData, completeStep , submitForCosting } = useItinerary();
  const { suppliers } = useSRM();
  ...
  const handleNext = async () => {
      ...
      if (user?.role === 'employee') {
          submitForCosting(); // 👈 Set status to 'pending_costing'
          // Optional: Redirect to library or stay here
          setTimeout(() => router.push('/dashboard/itinerary/library'), 1500);
      } else {
          // If Admin, just go to costing like normal
          router.push('/dashboard/itinerary/costing');
      }
  };
  return (
    <div>
      ...
      <div className="flex justify-between items-center mr-5 mb-5 ">
         ...
         {/* Find the bottom button section */}
<button 
    onClick={handleNext} 
