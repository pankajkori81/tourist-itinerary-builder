...
//   Calendar, Trash2, Edit, User, Clock, DollarSign,
//   Star, Moon, Car, Plane, Train, Ship, ArrowRight, ArrowLeft,
//   Ban, CheckCircle2, PlusSquare, Briefcase 
// } from 'lucide-react';
// import { useItinerary } from '@/app/context/ItineraryContext';
...
//                             {/* DETAILS GRID (Modified: Removed Cost Column) */}
...
//                               {/* DETAILS GRID (Modified: Removed Cost Column, Redistributed Spans) */}
//                               <div className="grid grid-cols-12 gap-3 mt-4">
//                                   <div className="col-span-4 bg-white border border-gray-200 rounded p-2">
//                                       <div className="text-[10px] text-gray-500 uppercase font-bold mb-0.5">Room</div>
//                                       <div className="text-xs font-bold text-gray-700 truncate" title={item.roomCategory}>
...
//                                 {/* RESTORED LOGISTICS (Cost Grid Removed) */}
//                                 <div className="mt-2 grid grid-cols-12 gap-3 mb-1">
...
//   // // 👇 HIGHLIGHT: Both Employees AND Agents get locked out when pending or approved
//   // const isLocked = 
//   //     (user?.role === 'employee' || user?.role === 'agent') && 
//   //     (itineraryData.status === 'pending_costing' || itineraryData.status === 'approved');


 
//   // 👇 HIGHLIGHT FIX 1: We define currentStatus at the top level to prevent "undefined" bugs!
//   const currentStatus = itineraryData.status || 'draft';

//   // 👇 HIGHLIGHT: Now we use currentStatus safely
//   const isLocked = 
//       (user?.role === 'employee' || user?.role === 'agent') && 
//       (currentStatus === 'pending_costing' || currentStatus === 'approved');
...
//     // Force price sanitization (Just in case)
...
//   // 👇 HIGHLIGHT FIX 2: handleNext uses the safe currentStatus and guarantees the alert fires
...
//               } else if (currentStatus === 'pending_costing') {
//                   alert("Please wait for the Admin to approve the costing.");
...
//       {/* 👇 1. REJECTION BANNER (Only if draft + has comment) */}
//       {itineraryData.status === 'draft' && itineraryData.adminComment && (
...
//       {/* 👇 3. LOCKING BANNER (If pending or approved) */}
//       {isLocked && (
...
//       {itineraryData.status === 'reedit_requested' && (
//         <div className="mx-4 mt-4 bg-orange-50 border-l-4 border-orange-500 p-4 rounded-r shadow-sm">
//            <p className="font-bold text-orange-800 text-sm">Request Sent</p>
//            <p className="text-orange-700 text-xs">Waiting for Admin to unlock this itinerary.</p>
//         </div>
//       )}
...
//                              {/* 👇 HIDE EDIT/DELETE BUTTONS IF LOCKED */}
//                                {/* {!isLocked && (
//                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
//                                       <button onClick={() => openEdit(item, 'add_activity')} className="text-blue-600 hover:bg-blue-50 p-1.5 rounded-md"><Edit size={16}/></button>
//                                       <button onClick={() => handleDeleteItem('activity', item.id)} className="text-red-600 hover:bg-red-50 p-1.5 rounded-md"><Trash2 size={16}/></button>
...
//             </div>
//           ) : (
//             // --- SUB FORMS RENDER (NO PRICES PASSED) ---
//             <div className="h-full p-6">
//                 {viewMode === 'add_activity' && <ActivityForm initialData={editingItem} existingActivities={currentDay.activities} city={currentDay.city} dayDate={currentDay.date} onSave={(d) => handleSaveItem('activity', d)} onCancel={() => setViewMode('list')} />}
//                 {viewMode === 'add_stay' && <StayForm initialData={editingItem} city={currentDay.city} dayDate={currentDay.date} onSave={(d) => handleSaveItem('stay', d)} onCancel={() => setViewMode('list')} />}
//                 {viewMode === 'add_transport' && <TransportForm initialData={editingItem} city={currentDay.city} dayDate={currentDay.date} onSave={(d) => handleSaveItem('transport', d)} onCancel={() => setViewMode('list')} />}
...
//              {user?.role === 'employee' && itineraryData.status === 'approved' && (
//                  <button 
...
//                 onClick={handleNext} 
//                 disabled={
//                     (user?.role === 'employee' && itineraryData.status === 'pending_costing') ||
//                     (user?.role === 'employee' && itineraryData.status === 'reedit_requested') // Disable if waiting
//                 }
...
//                 ${(user?.role === 'employee' && (itineraryData.status === 'pending_costing' || itineraryData.status === 'reedit_requested'))
//                     ? 'bg-gray-600 text-gray-400 cursor-not-allowed border border-gray-600' // Disabled State
//                     : (user?.role === 'employee' && itineraryData.status === 'draft'
//                         ? 'bg-green-600 hover:bg-green-500 text-white shadow-green-900/20' // Submit State
//                         : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/20') // Proceed State (Admin or Approved)
//                 }`}
//              >
...
//                 {/* {user?.role === 'employee' ? (
//                     <>
//                        {itineraryData.status === 'draft' && <>Submit for Costing <Send size={18} /></>}
//                        {(itineraryData.status === 'pending_costing' || itineraryData.status === 'reedit_requested') && <>Submitted for Costing <Clock size={18} /></>}
                       
                     
//                        {itineraryData.status === 'approved' && <>View Costing <ArrowRight size={18} /></>}
//                     </>
//                 ) : (
...
//                 {/* {user?.role === 'employee' || user?.role === 'agent' ? (
//                     <>
//                        {itineraryData.status === 'draft' && <>Request Pricing <Send size={18} /></>}
//                        {(itineraryData.status === 'pending_costing' || itineraryData.status === 'reedit_requested') && <>Pricing Requested <Clock size={18} /></>}
//                        {itineraryData.status === 'approved' && <>View Costing <ArrowRight size={18} /></>}
//                     </>
//                 ) : (
//                     <>Proceed to Costing <ArrowRight size={18} /></>
...
//          <div className="flex gap-3">
//              {/* 1. REQUEST RE-EDIT BUTTON (Visible if Approved and Employee) */}
//              {user?.role === 'employee' && itineraryData.status === 'approved' && (
//                  <button 
//                     onClick={handleRequestReEdit}
//                     className="px-6 py-3 text-red-600 bg-red-50 hover:bg-red-100 font-bold rounded-full border border-red-200 transition-all flex items-center gap-2"
...
//              {/* 👇 HIGHLIGHT: Define currentStatus here for the button UI */}
//              {(() => {
//                  const currentStatus = itineraryData.status || 'draft';
//                  const isAgentOrEmp = user?.role === 'employee' || user?.role === 'agent';
...
//                 }
//                 className={`group flex items-center gap-2 px-8 py-3 rounded-full font-semibold shadow-lg transition-all transform hover:scale-[1.02]
//                 ${((user?.role === 'employee' || user?.role === 'agent') && (currentStatus === 'pending_costing' || currentStatus === 'reedit_requested'))
//                     ? 'bg-gray-600 text-gray-400 cursor-not-allowed border border-gray-600' 
//                     : ((user?.role === 'employee' || user?.role === 'agent') && currentStatus === 'draft'
...
//                 {user?.role === 'employee' || user?.role === 'agent' ? (
//                     <>
//                        {currentStatus === 'draft' && <>Request Pricing <Send size={18} /></>}
//                        {(currentStatus === 'pending_costing' || currentStatus === 'reedit_requested') && <>Pricing Requested <Clock size={18} /></>}
//                        {currentStatus === 'approved' && <>View Costing <ArrowRight size={18} /></>}
//                     </>
//                 ) : (
//                     <>Proceed to Costing <ArrowRight size={18} /></>
//                 )}
...
//   // // 👇 GUARANTEED FIX: Define currentStatus at the top level
//   // const currentStatus = itineraryData.status || 'draft';

//   // // 👇 Both Employees AND Agents get locked out when pending or approved
//   // const isLocked = 
//   //     (user?.role === 'employee' || user?.role === 'agent') && 
//   //     (currentStatus === 'pending_costing' || currentStatus === 'approved');
...
//   // 👇 HIGHLIGHT FIX 1: Completely separated the conditional logic for Employee and Agent
//   const currentStatus = itineraryData.status || 'draft';
...
//   // 👇 GUARANTEED FIX: Fallback logic ensures alerts and redirects work for any status
//   // const handleNext = async () => {
//   //     completeStep('createDay');
...
//   //                 router.push('/dashboard/itinerary/costing');
//   //             } else {
//   //                 // Catch-all for 'draft', 'active', or any undefined state
//   //                 submitForCosting(); 
//   //                 alert("Pricing Request sent to Admin! Waiting for approval.");
//   //             }
//   //         } 
