...
//     // Initialize running date
//     let runningDate = startDateStr ? new Date(startDateStr) : new Date();

//     // 1. LOOP THROUGH ROUTES (Covers the "Nights")
//     routes.forEach((route) => {
//       const nightsCount = parseInt(route.nights?.toString() || '0') || 0;
      
//       // If nights is 0, we treat it as a day visit (1 day), otherwise we loop for the number of nights
//       const loopCount = nightsCount === 0 ? 1 : nightsCount; 
...
//     // 2. *** THE FIX: ADD THE FINAL CHECKOUT DAY (Day 7) ***
//     // After the loop finishes, we need one last day for the departure from the last city.
//     if (routes.length > 0) {
...
//             dayNumber: globalDayCounter,
//             date: dateString,
//             // The city is still the last city (where they wake up to check out)
...
//   // --- NEW LOGIC: CALCULATE GHOST STAYS (CONTINUED STAYS) ---
//   const getGhostStays = () => {
//     const ghosts: any[] = [];
    
//     // Loop through all days BEFORE the current selected day
//     for (let i = 0; i < selectedDayIndex; i++) {
//       const pastDay = dayPlans[i];
      
//       pastDay.stays.forEach(stay => {
//         // Calculate how many days into the trip this stay covers
//         // Logic: (Past Day Index + Nights) > Current Day Index
//         const stayEndIndex = i + stay.nights;
        
//         if (stayEndIndex > selectedDayIndex) {
//           // This stay is still active today!
//           ghosts.push({
//             ...stay,
//             isContinued: true, // <--- MARK AS GHOST
//             id: `ghost-${stay.id}-${selectedDayIndex}` // Unique ID for React key
//           });
//         }
//       });
//     }
...
//                   {/* --- ACTIVITY CARD --- */}
//                   {item.type === 'activity' && (
//                     <div className="bg-white rounded-xl p-5 shadow-lg border-l-4 border-blue-500 flex gap-5">
                       
//                        <div className="w-24 bg-blue-50 rounded-lg flex-shrink-0 flex flex-col items-center justify-center text-blue-600 border border-blue-100">
//                            <Camera size={32} />
//                        </div>
...
//                                  {item.heading}
//                                  {item.inclusionType === 'optional' && (
//                                    <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full border border-amber-200">Optional</span>
//                                  )}
//                                </h4>
...
//                              <div className="w-24  mt-15 rounded-lg flex-shrink-0 flex flex-col items-center justify-center text-orange-600 border border-orange-100">
//                              <Hotel size={24} />
//                               </div>
                            
//                             {/* BADGE LOGIC */}
//                             <div className={`absolute top-0 left-0 text-white text-[10px] font-bold px-1 py-1 rounded-br-lg z-10 ${
//                                 item.isContinued ? 'bg-gray-500' : 'bg-purple-600'
//                             }`}>
...
//                                         <span className="text-[10px] font-bold uppercase mt-2 tracking-wider bg-purple-50 text-gray-900 px-2 py-0.5 rounded border border-purple-100">
//                                             {item.category} • {item.stayType}
//                                         </span>
//                                         <span className="text-[10px] font-bold uppercase mt-2 tracking-wider bg-orange-50 text-gray-900 px-2 py-0.5 rounded border border-orange-100 flex items-center gap-1">
//                                         <Utensils size={10}/> {item.mealPlan}
//                                         </span>
...
//                                     item.isContinued ? 'bg-gray-100 border-gray-200' : 'bg-green-50 border-green-100'
//                                 }`}>
//                                     <div className={`text-[10px] uppercase font-bold mb-0.5 ${
//                                         item.isContinued ? 'text-gray-500' : 'text-green-600'
//                                     }`}>
//                                         {item.isContinued ? 'Cost Status' : 'Total Cost'}
//                                     </div>
//                                     <div className={`text-sm font-bold ${
//                                         item.isContinued ? 'text-gray-600 italic' : 'text-green-800'
//                                     }`}>
//                                         {item.isContinued 
...
//                   {/* --- TRANSPORT CARD --- */}
//                   {/* { item.type === 'transport' && (
//                     <div className="bg-white rounded-xl p-5 shadow-lg border-l-4 border-green-500 flex gap-5">
//                        <div className="w-24 h-24 bg-gray-200 rounded-lg flex-shrink-0 relative overflow-hidden">
//                              <div className="absolute inset-0 bg-green-600 flex items-center justify-center text-white font-bold text-xs">TRANS</div>
//                        </div>
//                        <div className="flex-1">
//                           <div className="flex justify-between">
//                              <h4 className="font-bold text-xl text-gray-800">
//                                 {item.serviceMode === 'package' ? item.selectedPackage : item.selectedIndividualService}
//                              </h4>
//                              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
//                                 <button onClick={() => openEdit(item, 'add_transport')} className="text-green-600 hover:bg-green-50 p-1 rounded"><Edit size={16}/></button>
//                                 <button onClick={() => handleDeleteItem('transport', item.id)} className="text-red-600 hover:bg-red-50 p-1 rounded"><Trash2 size={16}/></button>
//                              </div>
//                           </div>
//                           <div className="flex gap-2 mt-1">
//                               <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded capitalize">{item.vehicleType}</span>
//                               <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded uppercase">{item.mode}</span>
//                           </div>
//                           <div className="mt-3 flex gap-4 text-xs text-gray-500 bg-gray-50 p-2 rounded">
//                              <div><strong>Pickup:</strong> {item.pickupLocation}</div>
//                              <div><strong>Drop:</strong> {item.dropoffLocation}</div>
...
//                   {item.type === 'transport' && (
//                     <div className="bg-white rounded-xl p-5 shadow-lg border-l-4 border-green-500 flex gap-5 group">
                       
//                        {/* Left: Dynamic Icon based on Mode */}
//                        <div className="w-24 bg-green-50 rounded-lg flex-shrink-0 flex flex-col items-center justify-center text-green-600 border border-green-100">
...
//                                    <span className="text-[10px] bg-gray-100 text-gray-600 px-5 py-0.5 mt-1 rounded-full border border-gray-200 font-bold uppercase">
//                                       {item.subType}
//                                    </span>
//                                 </h4>
//                                 <div className="text-xs text-gray-600 mt-3 font-medium flex gap-3">
//                                     <span>Vehical Qty: <strong>{item.vehicleCount || 1}</strong></span>
...
//                               {/* Logistics Block */}
//                               <div className="col-span-6 bg-gray-50 p-5 rounded-lg border border-gray-100 flex flex-col justify-center">
//                                   {/* Pickup Line */}
//                                   {/* <div className="flex items-center gap-2 mb-1.5">
//                                       <div className="w-1.5 h-1.5 rounded-full bg-green-500 text-xs"></div>
//                                      <span className="text-xs font-bold text-gray-700 truncate ">  Pickup :   {item.pickupLocation}   </span>
//                                      <span className="text-xs font-bold text-gray-700 truncate ml-5 ">  Time: {item.pickupTime}hrs  </span>
//                                   </div> */}
...
//                     </div>
//                 </div>
//                                   {/* Logic: Show Dropoff OR Duration */}
//                                   {/* {item.subType === 'disposal' ? (
//                                       <div className="flex items-center gap-2 pl-3.5">
//                                           <span className="text-[10px] font-bold text-gray-400 uppercase">Duration:</span>
//                                           <span className="text-xs font-bold text-gray-700">{item.duration}</span>
//                                       </div>
//                                   ) : (
//                                       <div className="flex items-center gap-2">
//                                           <div className="w-1.5 h-1.5 rounded-full bg-red-400"></div>
//                                           <span className="text-xs font-bold text-gray-700 truncate">
//                                               {item.dropoffTime ? `${item.dropoffTime} • ` : ''}{item.dropoffLocation || 'Drop-off not set'}
//                                           </span>  
//                                           <span className="text-xs font-bold text-gray-700 truncate">
//                                               {item.dropoffTime ? `${item.dropoffTime} • ` : ''}{item.dropoffLocation || 'Drop-off not set'}
//                                           </span>
...
//                               {/* Status Badge */}
//                               <div className="col-span-3 bg-gray-50 p-2 rounded-lg border border-gray-100 flex flex-col justify-center items-center">
//                                   <div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Status</div>
//                                   <div className={`text-xs font-bold px-2 py-0.5 rounded ${
//                                       item.serviceType === 'Optional' 
...
//                               {/* Total Cost */}
//                               <div className="col-span-3 bg-gray-50 p-2 rounded-lg border border-gray-100 flex flex-col justify-center items-end pr-3">
//                                   <div className="text-[10px] text-gray-600 uppercase font-bold  flex items-center gap-1">
...
//                   {/* --- MEAL CARD (NEW) --- */}
//                   {item.type === 'meal' && (
//                     <div className="bg-white rounded-xl p-5 shadow-lg border-l-4 border-orange-500 flex gap-5 group">
                        
//                         {/* Icon */}
//                         <div className="w-24 bg-orange-50 rounded-lg flex-shrink-0 flex flex-col items-center justify-center text-orange-600 border border-orange-100">
//                              <Utensils size={24} />
//                         </div>
...
//                                 {/* Cost (Only if included) */}
//                                 <div className="col-span-4 bg-gray-50 p-3 mb-2 rounded-lg border border-gray-100 flex flex-col items-end">
//                                     <div className="text-[11px] text-gray-600 uppercase font-bold mb-0.5">Total Cost (PP)</div>
//                                     <div className="text-sm font-bold text-green-800">
//                                         {item.inclusionType === 'included' 
//                                           ? `$${item.adultCost}`
//                                           : <span className="text-gray-400 italic text-xs">As per menu</span>
//                                         }
//                                     </div>
//                                 </div>
//                             </div>
...
//                         existingActivities={currentDay.activities} // <--- VITAL: Passing the existing activities so validation works
...
//       {/* <div>
//         // Inside your main Itinerary Builder Page
// <CostingSheet itineraryData={days} />
//       </div> */}
                          ...
                          <div className="flex-1 flex flex-col justify-between">
                              ...
                              <div className="mt-5 grid grid-cols-12 gap-3">
                                  ...
                                  <div className={`col-span-4 p-3 mb-2 rounded-lg border flex flex-col items-end ${item.inclusionType === 'excluded' ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                                      <div className={`text-[11px] uppercase font-bold mb-0.5 ${item.inclusionType === 'excluded' ? 'text-red-600' : 'text-gray-600'}`}>Total Cost (PP)</div>
