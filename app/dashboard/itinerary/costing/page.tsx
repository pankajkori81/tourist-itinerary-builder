...
// import { useCurrency } from '@/hooks/useCurrency';
// import { DayPlan } from '../create-day/constants/daywiseConstants';
// import { FixedDeparture } from '@/utils/itineraryStorage'; // Import the new type
...
// // ... (Keep getShareLabel and other existing helpers here) ...
// const getShareLabel = (pax: number) => {
//   if (pax === 1) return "Single";
//   if (pax === 2) return "Twin/Double";
//   if (pax === 3) return "Triple";
...
//     // Load existing departures if any
//     if (itineraryData.fixedDepartures) {
//       setFixedDepartures(itineraryData.fixedDepartures);
//     }
//   }, [itineraryData]);

//   // --- COST CALCULATION ENGINE (Legacy) ---
//   // ... (Keep your processedDayPlans and calculatedNetTotal logic exactly as it was) ...
//   const processedDayPlans = useMemo(() => {
//     const plansWithGhosts = JSON.parse(JSON.stringify(rawDayPlans)) as DayPlan[];
//     rawDayPlans.forEach((day) => {
//         if (!day.stays) return;
...
//   // --- NEW LOGIC: PRICING STRATEGY ---
//   const netInSelected = convert(calculatedNetTotal, currency);
  
//   // Find if a fixed price is active
...
//     // === OVERRIDE MODE ===
//     // If a fixed price is selected, that IS the price.
//     finalPerPerson = activeFixedDeparture.price;
//     finalGrandTotal = finalPerPerson * travelerCount;
    
//     // Reverse Calculate Margin: (Selling Price - Net) = Margin
//     const netPerPerson = travelerCount > 0 ? netInSelected / travelerCount : 0;
//     displayMarkupAmount = finalGrandTotal - netInSelected;
    
//     // Calculate what the % would be
//     if (netInSelected > 0) {
//         displayMarkupPercent = (displayMarkupAmount / netInSelected) * 100;
//     } else {
//         displayMarkupPercent = 100;
//     }
//   } else {
...
//     finalGrandTotal = finalPerPerson * travelerCount;
//     displayMarkupAmount = markupAmount;
//   }


//   // --- HANDLERS FOR FIXED DEPARTURES ---
...
//         if (d.id === id) {
//             // Auto-generate label if date changes
//             if (field === 'date') {
//                 const dateObj = new Date(value);
//                 // Format: Fri, Jan 09, 2026
//                 const label = isNaN(dateObj.getTime()) ? value : dateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: '2-digit', year: 'numeric' });
//                 return { ...d, [field]: value, label: label };
//             }
...
//     // Toggle logic: If clicking already selected, deselect it. Otherwise select it.
//     const isCurrentlySelected = fixedDepartures.find(d => d.id === id)?.isSelected;
    
//     const updated = fixedDepartures.map(d => ({
//         ...d,
//         isSelected: d.id === id ? !isCurrentlySelected : false // Only one can be selected
//     }));
    
//     setFixedDepartures(updated);
//     // Important: Update context so Preview page knows
//     updateItineraryData({ 
//         fixedDepartures: updated,
//         useFixedPrice: !isCurrentlySelected 
//     });
//   };
...
//   // ... (keep excel/pdf download functions same as before) ...
//   const handleDownloadExcel = () => { /* ... existing logic ... */ };
...
//         {/* ================= LEFT: THE MASTER LEDGER TABLE & DATES ================= */}
//         <div className="flex-1 w-full space-y-6">
            
//             {/* 1. LEDGER TABLE (Same as before) */}
//             <div className="bg-white border border-gray-300 rounded-xl shadow-sm overflow-hidden">
//                 <div className="overflow-x-auto">
//                     <table className="w-full text-left border-collapse">
//                         {/* ... (Keep your existing Table Header & Body here) ... */}
//                         {/* To save space in this answer, I am assuming the Ledger Table code 
//                             from previous step remains exactly here.
//                          */}
//                           <thead className="bg-gray-50 text-gray-500 border-b border-gray-300">
//                             <tr>
//                                 <th className="py-3 px-4 text-xs font-bold uppercase w-[80px]">Type</th>
...
//                         </thead>
//                          <tbody className="text-sm">
//                            {/* ... Your Existing ProcessedDayPlans Map Logic ... */}
//                             {processedDayPlans.map((day) => {
...
//                                                 <div className="flex items-center gap-2 text-gray-700 font-bold">
//                                                     <Calendar size={14} className="text-blue-500"/>
//                                                     <span>DAY {day.dayNumber} - {day.city || "Unknown"}</span>
//                                                 </div>
//                                             </td>
//                                         </tr>
//                                         {/* ... Render Stays, Transports etc using LedgerRow ... */}
//                                         {/* I am not repeating the whole map here to keep answer clean, but retain it! */}
//                                     </React.Fragment>
//                                 );
//                             })}
//                          </tbody>
...
//             {/* 2. NEW SECTION: FIXED DEPARTURES & PRICING TABLE */}
//             <div className="bg-white border border-gray-300 rounded-xl shadow-sm overflow-hidden p-6">
...
//                         className="text-xs flex items-center gap-1 bg-blue-50 text-blue-600 px-3 py-1.5 rounded-md font-bold hover:bg-blue-100 transition"
//                     >
...
//                                         No fixed dates added. Click "Add Date" to create packages.
//                                     </td>
//                                 </tr>
//                             )}
...
//                                         <button 
//                                             onClick={() => selectDepartureRow(row.id)}
//                                             className={`transition-all ${row.isSelected ? 'text-blue-600 scale-110' : 'text-gray-300 hover:text-gray-400'}`}
//                                         >
//                                             {row.isSelected ? <CheckCircle size={20} fill="currentColor" className="text-white bg-blue-600 rounded-full" /> : <Circle size={20} />}
//                                         </button>
//                                     </td>
...
//                                             type="date" 
//                                             value={row.date} 
//                                             onChange={(e) => updateDepartureRow(row.id, 'date', e.target.value)}
//                                             className="w-full border border-gray-200 rounded px-2 py-1.5 text-gray-700 focus:border-blue-500 outline-none"
//                                         />
//                                     </td>
//                                     <td className="py-3 px-4">
//                                          <input 
//                                             type="text" 
//                                             value={row.label} 
//                                             onChange={(e) => updateDepartureRow(row.id, 'label', e.target.value)}
//                                             placeholder="e.g. Fri, Jan 09, 2026"
//                                             className="w-full border border-gray-200 rounded px-2 py-1.5 text-gray-700 bg-transparent focus:border-blue-500 outline-none"
//                                         />
//                                     </td>
//                                     <td className="py-3 px-4">
//                                         <div className="flex items-center gap-2">
...
//                                                 type="number" 
//                                                 value={row.price} 
//                                                 onChange={(e) => updateDepartureRow(row.id, 'price', parseFloat(e.target.value))}
//                                                 className="w-full border border-gray-200 rounded px-2 py-1.5 font-mono font-bold text-gray-800 focus:border-blue-500 outline-none"
//                                             />
//                                         </div>
//                                     </td>
//                                     <td className="py-3 px-4">
//                                         <select 
//                                             value={row.status}
//                                             onChange={(e) => updateDepartureRow(row.id, 'status', e.target.value)}
//                                             className="w-full border border-gray-200 rounded px-2 py-1.5 text-xs font-bold text-gray-600 bg-white"
//                                         >
...
//                                     <td className="py-3 px-4 text-center">
//                                         <button 
//                                             onClick={() => removeDepartureRow(row.id)}
...
//                             {activeFixedDeparture ? 'Fixed Package Calculator' : 'Quote Calculator'}
//                         </span>
//                     </div>
...
//                                 <input 
//                                     type="number" 
//                                     value={activeFixedDeparture ? displayMarkupPercent.toFixed(1) : markupPercent} 
//                                     onChange={(e) => !activeFixedDeparture && handleMarkupChange(parseFloat(e.target.value) || 0)} 
//                                     disabled={!!activeFixedDeparture}
//                                     className={`w-full p-2 border rounded-lg font-bold text-sm ${activeFixedDeparture ? 'bg-gray-100 text-gray-500 border-gray-200' : 'bg-white border-gray-300 text-gray-800'}`}
//                                 />
...
//                                     {activeFixedDeparture ? 'Fixed Package Price' : 'Selling Price / Per Person'}
//                                 </div>
//                                 <div className="font-mono font-black text-3xl tracking-tight">{formatPrice(finalPerPerson, currency)}</div>
...
export default function CostingPage() {
  ...
  // --- DATA PROCESSING: INJECT GHOST STAYS (MULTI-NIGHT LOGIC) ---
  const processedDayPlans = useMemo(() => {
    const plansWithGhosts = JSON.parse(JSON.stringify(rawDayPlans)) as DayPlan[];
  ...
  const selectDepartureRow = (id: string) => {
    ...
    const updated = fixedDepartures.map(d => ({
        ...d,
        isSelected: d.id === id ? !isCurrentlySelected : false 
    }));
    setFixedDepartures(updated);
    updateItineraryData({ 
        fixedDepartures: updated,
  ...
  // --- EXPORT FUNCTIONS (Simplified placeholders to match structure) ---
  const handleDownloadExcel = () => { 
      // Your excel logic here... 
      // Ensure you pass processedDayPlans to include the main table data
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans text-gray-800">
      ...
      <main className="max-w-[1600px] mx-auto p-6 flex flex-col xl:flex-row gap-6 items-start">
        ...
        {/* ================= LEFT SIDE: MAIN LEDGER + FIXED DEPARTURES ================= */}
        <div className="flex-1 w-full space-y-6">
            
            {/* 1. MASTER LEDGER TABLE (RESTORED LOGIC) */}
            ...
            {/* 2. FIXED DEPARTURES & PRICING TABLE (BELOW MAIN TABLE) */}
            <div className="bg-white border border-gray-400 rounded-xl shadow-md overflow-hidden p-6">
                 <div className="flex justify-between items-center mb-4">
                    ...
                    <button 
                        onClick={addDepartureRow}
                        className="text-sm flex items-center gap-1 bg-blue-50 text-blue-100 px-3 py-1.5 rounded-md font-bold hover:bg-blue-500 transition bg-blue-600"
                    >
    ...
    >
        ...
        <div className={`
            ...
            ${row.isSelected 
                ? 'bg-blue-600 border-blue-600 scale-110' // Selected: Solid Blue Fill
                : 'bg-white border-gray-300 group-hover:border-gray-400' // Unselected: White background, Gray Ring
            }
        `}>
            {/* The Checkmark Icon (Only shows when selected) */}
            {row.isSelected && (
                <Check size={14} className="text-white" strokeWidth={3} />
                                    ...
                                    <td className="py-3 px-4">
                                        <input 
                                            ...
                                            value={row.date} 
                                            onChange={(e) => updateDepartureRow(row.id, 'date', e.target.value)}
                                            className="w-full border border-gray-300 rounded px-2 py-1.5 text-gray-700 focus:border-blue-500 outline-none"
                                        />
                                    </td>
                                    <td className="py-3 px-4">
                                         <input 
                                            ...
                                            onChange={(e) => updateDepartureRow(row.id, 'label', e.target.value)}
                                            placeholder="e.g. Fri, Jan 09, 2026"
                                            className="w-full border border-gray-300 rounded px-2 py-1.5 text-gray-700 bg-transparent focus:border-blue-500 outline-none"
                                        />
                                    </td>
                                    <td className="py-3 px-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input 
                                                ...
                                                value={row.price} 
                                                onChange={(e) => updateDepartureRow(row.id, 'price', parseFloat(e.target.value))}
                                                className="w-full border border-gray-300 rounded px-2 py-1.5 font-mono font-bold text-gray-800 focus:border-blue-500 outline-none"
                                            />
                                        </div>
                                    </td>
                                    <td className="py-3 px-4">
                                        <select 
                                            value={row.status}
                                            onChange={(e) => updateDepartureRow(row.id, 'status', e.target.value)}
                                            className="w-full border border-gray-300 rounded px-2 py-1.5 text-xs font-bold text-gray-600 bg-white"
                                        >
                                            <option value="Open">Open</option>
                                            <option value="Filling Fast">Filling Fast</option>
                                            <option value="Sold Out">Sold Out</option>
...
const LedgerRow = ({ 
    ...
    let rowClass = "transition-colors odd:bg-white even:bg-gray-50/50";
    let textClass = "font-medium text-sm block text-gray-800";
    let costClass = "font-mono text-sm font-medium text-gray-500";

    if (isLastInGroup) {
        rowClass += " border-b border-gray-400"; // Stronger border to separate sections (Stay vs Transport)
    } else {
        rowClass += " border-b-0"; // No border for internal rows (merges Twin & Quad)
    }
    ...
    return (
        <tr className={rowClass}>
            {!isSubRow && (
                <td 
                    rowSpan={rowSpan} 
                    className="py-3 px-4 align-top w-[90px] border-r border-gray-400" 
                >
                     <span className={`text-[11px] font-bold uppercase tracking-wider ${typeColor || "text-gray-500"}`}>
                        {typeLabel}
                    </span>
