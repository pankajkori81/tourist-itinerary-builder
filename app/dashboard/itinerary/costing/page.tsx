  ...
  const handleDownloadPDF = () => {
    ...
    processedDayPlans.forEach(day => {
        ...
        if(day.meals) day.meals.forEach(m => {
            const adults = typeof m.paxAdult === 'number' ? m.paxAdult : travelerCount;
            const children = typeof m.paxChild === 'number' ? m.paxChild : 0;
            const totalCost = (safeNum(m.adultCost) * adults) + (safeNum(m.childCost) * children);
            addRow('Meal', m.restaurantName, m.inclusionType, m.mealType, totalCost);
        });
    });
    ...
    doc.text(`Markup (${markupPercent}%):`, 150, currentY, { align: "right" });
    doc.setTextColor(0, 0, 0);
    // Use the calculated markup variable here to fix scope issue
                                ...
                                return (
                                    <React.Fragment key={day.dayNumber}>
                                        ...
                                        {(day.transports || []).map((t: any, i: number) => (
                                            <LedgerRow 
                                                key={`t-${i}`} typeLabel="Transport" typeColor="text-gray-900"
                                                details={t.vehicleType} inclusionType={t.inclusionType} config={`${t.vehicleCount} Vehicles`}
                                                rawCost={safeNum(t.price) * safeNum(t.vehicleCount)} travelerCount={travelerCount} currency={currency} convert={convert} formatPrice={formatPrice}
                                        ...
                                        {(day.activities || []).map((a: any, i: number) => (
                                            <LedgerRow 
                                                key={`a-${i}`} typeLabel="Activity" typeColor="text-gray-900"
                                                details={a.heading} inclusionType={a.inclusionType} config={`${travelerCount} Pax`}
                                                rawCost={((safeNum(a.entranceFeePP) + safeNum(a.activityFeePP)) * travelerCount) + (a.guideType === 'guided' ? safeNum(a.guideFee) : 0)}
