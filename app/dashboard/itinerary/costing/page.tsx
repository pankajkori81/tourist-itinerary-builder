...
export default function CostingPage() {
  ...
  const syncSmartRates = () => {
      if (!simulationDate) {
          alert("Please set a 'Valid From / Ref. Start Date' (top right) so the engine knows what dates to calculate!");
          return;
      }
      ...
      rawDayPlans.forEach(day => {
          ...
          const getBestSeason = (supplierId?: string, itemName?: string) => {
              let tariff;
              if (supplierId) tariff = allTariffs.find(t => t.serviceId === supplierId);
              if (!tariff && itemName) tariff = allTariffs.find(t => t.serviceName === itemName);
          ...
          day.stays?.forEach(s => {
              ...
              if (bestSeason && bestSeason.rates && bestSeason.rates.length > 0) {
                  ...
                  const targetRoomName = s.roomCategory || s.roomName || 'Standard Room';
                  let matchedRoom = bestSeason.rates.find((r: any) => 
      ...
      if (ratesFound > 0) {
          alert(`Success! Auto-filled ${ratesFound} live rates from the Rate Manager based on exact occupancy and dates.`);
      } else {
          alert("No matching seasonal rates found for these dates. Any items left at $0 require manual pricing.");
      }
  };
...
/>
    ...
    <div className="flex items-center gap-2">
        ...
        <span className="font-bold tracking-wide text-sm">
            {activeFixedDeparture ? 'Fixed Package Price' : `Quote Calculator (${selectedMonth})`}
        </span>
    </div>
    ...
    {!activeFixedDeparture && user?.role === 'admin' && (
        ...
        >
            {isSyncingRates ? <Loader2 size={14} className="animate-spin"/> : <Sparkles size={14} className="text-yellow-300"/>}
            Sync Smart Rates
        </button>
    )}
</div>
