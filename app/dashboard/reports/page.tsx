...
// //   topSuppliers: { _id: string; name: string; totalSpend: number; leakage: number }[];
// // }


// interface OpsPayload {
//   success: boolean;
//   // NEW: Executive KPI Ribbon Data
//   kpis: {
//     totalSpend: number;
//     totalLeakage: number;
...
//   };
//   // NEW: Time-series data for the ComposedChart
//   trendData: {
//     month: string;
//     expectedCost: number;
//     actualCost: number;
//   }[];
//   spendByType: { name: string; value: number; color: string }[];
//   // UPDATED: Added totalBookings and variance percentage to suppliers
//   topSuppliers: { 
//     _id: string; 
//     name: string; 
//     totalSpend: number; 
...
// const [activeTab, setActiveTab] = useState<"financials" | "sales" | "operations">("financials");
...
// // Add the state to hold the fetched data
// const [opsData, setOpsData] = useState<OpsPayload | null>(null);
...
// // --- Optimized Fetch Logic (Lazy Loading) ---
//   useEffect(() => {
...
//           if (json.success) setFinData(json);
          
//         } else if (activeTab === "sales") {
//           const res = await fetch(`/api/reports/sales?timeframe=${timeframe}`);
//           const json = await res.json();
//           if (json.success) setSalesData(json);
          
//         } else if (activeTab === "operations") {
//           const res = await fetch(`/api/reports/suppliers?timeframe=${timeframe}`);
//           const json = await res.json();
//           if (json.success) setOpsData(json);
//         }
//       } catch (error) {
...
//     fetchData();
//   }, [timeframe, activeTab]); // Re-runs when the date changes or they click a new tab
...
//     className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "operations" ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
//   >
//     Operations & SRM
//   </button>
...
//           /* ════════════════════════════════════════════════════════════ */
//           /* PHASE 3: OPERATIONS & SRM DASHBOARD                          */
//           /* ════════════════════════════════════════════════════════════ */
//           <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            
//             {/* 1. Spend by Service Type (Pie Chart) */}
//             {/* 1. Spend by Service Type (Pie Chart) */}
//     <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200 lg:col-span-1 flex flex-col">
//       <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">Spend Distribution</h3>
//       <div className="w-full h-[300px] flex items-center justify-center">
//         <ResponsiveContainer width="100%" height="100%">
//           <PieChart>
//             <Pie
//               data={opsData.spendByType}
//               cx="50%"
//               cy="50%"
//               innerRadius={70}
//               outerRadius={90}
...
//               stroke="none"
//             >
//               {opsData.spendByType.map((entry, index) => (
...
//         {opsData.spendByType.map((item) => (
//           <div key={item.name} className="flex items-center gap-2 text-xs font-bold text-slate-600 uppercase">
...
//             {/* 2. Top Suppliers & Leakage Table */}
//             <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200 lg:col-span-2">
//               <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">Top Suppliers & Cost Leakage</h3>
//               <div className="overflow-x-auto">
//                 <table className="w-full text-left border-collapse">
//                   <thead>
//                     <tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400">
//                       <th className="pb-4 font-bold">Supplier Name</th>
//                       <th className="pb-4 font-bold text-right">Total Spend</th>
//                       <th className="pb-4 font-bold text-right">Cost Leakage</th>
//                       <th className="pb-4 font-bold text-center">Status</th>
//                     </tr>
//                   </thead>
//               <tbody>
//                     {opsData.topSuppliers.map((supplier) => (
//                       <tr key={supplier._id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors cursor-pointer group">
//                         <td className="py-4">
//                           <p className="font-bold text-slate-900 text-sm group-hover:text-indigo-600 transition-colors">{supplier.name}</p>
//                           {/* NEW: Transaction Volume Context */}
//                           <p className="text-[11px] font-semibold text-slate-400 mt-0.5">{supplier.totalBookings} Transactions</p>
//                         </td>
//                         <td className="py-4 text-right font-mono font-bold text-slate-700">
//                           {formatCurrency(supplier.totalSpend)}
//                         </td>
//                         <td className="py-4 text-right">
//                           <div className="flex flex-col items-end">
//                             <span className={`font-mono font-bold ${supplier.leakage > 0 ? "text-rose-600" : "text-emerald-600"}`}>
//                               {supplier.leakage > 0 ? "+" : ""}{formatCurrency(supplier.leakage)}
//                             </span>
//                             {/* NEW: Contextual Variance Percentage */}
//                             {supplier.leakage > 0 && (
//                               <span className="text-[10px] text-rose-400 font-bold">{supplier.variance.toFixed(1)}% variance</span>
//                             )}
//                           </div>
//                         </td>
//                         <td className="py-4 text-center">
//                           {/* UPDATED: Dynamic Enterprise Health Logic */}
//                           {supplier.variance <= 2 ? (
//                             <span className="bg-emerald-100 text-emerald-700 text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wide border border-emerald-200">Healthy</span>
//                           ) : supplier.variance <= 8 ? (
//                             <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wide border border-amber-200">Review</span>
//                           ) : (
//                             <span className="bg-rose-100 text-rose-700 text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wide border border-rose-200">At Risk</span>
//                           )}
//                         </td>
//                       </tr>
//                     ))}
...
import { 
  ...
  Activity, ArrowUpRight, ArrowDownRight, 
  Landmark, PlaneTakeoff, Building2, MapPin,
  Trophy, Filter, Users, Target, AlertTriangle, ShieldCheck
} from "lucide-react";
import { useRouter } from "next/navigation";
import { 
  AreaChart, Area, XAxis, YAxis, CartesianGrid, 
  Tooltip, ResponsiveContainer, 
  Pie, Cell, ComposedChart, Bar, Line
} from "recharts";
...
interface SalesPayload {
  ...
  timeframe?: string;
  kpis: { totalLeads: number; conversionRate: number; activeAgents: number; };
  funnel: { stage: string; count: number; value: number; color: string; }[];
  leaderboard: {
    monthlyTarget: number; agentId: string; name: string; avatar: string; totalRevenue: number; dealsClosed: number; 
  }[];
}
...
export default function EnterpriseReportsPage() {
  ...
  const handleExport = () => {
    ...
    if (activeTab === "financials" && finData) {
      ...
      worksheet['!cols'] = [{ wch: 10 }, { wch: 25 }, { wch: 15 }, { wch: 15 }, { wch: 20 }];
      XLSX.utils.book_append_sheet(workbook, worksheet, "Team Performance");
    } else if (activeTab === "operations" && opsData) {
       const opsSheetData = [
        ["TRAVDEK - OPERATIONS & SRM REPORT"],
        ["Exported on:", today, "Timeframe:", displayTimeframe],
        [], 
        ["SUPPLIER PERFORMANCE & LEAKAGE"],
        ["Supplier Name", "Transaction Volume", "Total Spend", "Cost Leakage", "Variance %"],
        ...opsData.topSuppliers.map(supplier => [
          supplier.name,
          supplier.totalBookings,
          `${currency}${supplier.totalSpend.toLocaleString()}`,
          `${currency}${supplier.leakage.toLocaleString()}`,
          `${supplier.variance.toFixed(1)}%`
        ])
      ];

      const worksheet = XLSX.utils.aoa_to_sheet(opsSheetData);
      worksheet['!cols'] = [{ wch: 30 }, { wch: 20 }, { wch: 15 }, { wch: 15 }, { wch: 15 }];
      XLSX.utils.book_append_sheet(workbook, worksheet, "Supplier Management");
    } else {
      return;
    }
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-[1400px] mx-auto w-full flex-1 flex flex-col">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          <div>
            ...
            <div className="flex items-center gap-2 mt-4 bg-slate-200/50 p-1 rounded-xl w-max">
              ...
              <button onClick={() => setActiveTab("operations")} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === "operations" ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                Operations & SRM
              </button>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-5">
                  {finData.breakdowns.byDestination.length > 0 ? (
                    finData.breakdowns.byDestination.map((dest, idx) => {
                      const maxProfit = finData.breakdowns.byDestination[0]?.profit || 1;
                      const widthPercent = ((dest.profit || 0) / maxProfit) * 100;
                      return (
                        <div key={idx}>
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-5">
                  {finData.breakdowns.byService.length > 0 ? (
                    finData.breakdowns.byService.map((srv, idx) => {
                      const maxCost = finData.breakdowns.byService[0]?.cost || 1;
                      const widthPercent = ((srv.cost || 0) / maxCost) * 100;
                      return (
                        <div key={idx}>
                          ...
                          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-rose-400 rounded-full" style={{ width: `${widthPercent}%` }} />
                          </div>
                        </div>
                      )
                    })
                          ...
                          <tr key={agent.agentId} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div><p className="font-bold text-slate-900 text-sm">{agent.name}</p></div>
                              </div>
                            </td>
                            <td className="py-4 text-center">
                              <span className="inline-flex items-center justify-center bg-slate-100 text-slate-600 font-bold text-xs px-3 py-1 rounded-full">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Operational Spend</h4>
                  <Wallet size={16} className="text-indigo-500" />
                </div>
                <div className="text-3xl font-black text-slate-900 font-mono">{formatCurrency(opsData.kpis.totalSpend)}</div>
                <p className="text-xs font-semibold text-slate-400 mt-2">Combined supplier costs</p>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Cost Leakage</h4>
                  <AlertTriangle size={16} className={opsData.kpis.totalLeakage > 0 ? "text-rose-500" : "text-emerald-500"} />
                </div>
                <div className="text-3xl font-black text-slate-900 font-mono">
                  {opsData.kpis.totalLeakage > 0 ? "+" : ""}{formatCurrency(opsData.kpis.totalLeakage)}
                </div>
                <p className="text-xs font-semibold text-slate-400 mt-2">Variance above expected cost</p>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Active Suppliers</h4>
                  <Building2 size={16} className="text-amber-500" />
                </div>
                <div className="text-3xl font-black text-slate-900 font-mono">{opsData.kpis.activeSuppliers}</div>
                <p className="text-xs font-semibold text-slate-400 mt-2">Unique partners engaged</p>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Overall Variance</h4>
                  <ShieldCheck size={16} className={opsData.kpis.variancePercent > 5 ? "text-rose-500" : "text-emerald-500"} />
                </div>
                <div className="text-3xl font-black text-slate-900 font-mono">{opsData.kpis.variancePercent.toFixed(2)}%</div>
                <p className="text-xs font-semibold text-slate-400 mt-2">Across all operations</p>
              </div>
            </div>

            {/* ROW 2: Spend Trend Analysis */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200 w-full h-[400px] flex flex-col">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-900">Spend vs. Invoice Over Time</h3>
                  <p className="text-xs font-semibold text-slate-500 mt-1">Identify which months experienced the highest cost leakages.</p>
                </div>
                <div className="flex items-center gap-4 text-xs font-bold text-slate-600 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 w-max">
                  <span className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-slate-300" /> Expected Net Cost</span>
                  <span className="flex items-center gap-1.5"><div className="w-3 h-1 rounded bg-rose-500" /> Actual Invoiced Amount</span>
                </div>
              </div>
              
              <div className="flex-1 w-full h-full min-h-[250px]">
                {opsData.trendData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={opsData.trendData} margin={{ top: 20, right: 20, bottom: 0, left: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                      <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12, fontWeight: 600 }} dy={10} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12, fontWeight: 600 }} tickFormatter={(val) => `$${val >= 1000 ? (val / 1000).toFixed(0) + 'k' : val}`} />
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: 'transparent' }} />
                      <Bar dataKey="expectedCost" name="Expected Cost" fill="#cbd5e1" radius={[4, 4, 0, 0]} maxBarSize={60} />
                      <Line type="monotone" dataKey="actualCost" name="Actual Invoiced" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, fill: '#ef4444', strokeWidth: 0 }} activeDot={{ r: 6 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="w-full h-full flex items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl">
            ...
            {/* ROW 3: Pie Chart and Updated Table */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200 lg:col-span-1 flex flex-col">
                ...
                <div className="w-full h-[300px] flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={opsData.spendByType} cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={5} dataKey="value" stroke="none">
                        {opsData.spendByType.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Pie>
                      <Tooltip formatter={(value: any, name: any) => [`$${Number(value).toLocaleString()}`, name]} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                <div className="flex flex-wrap justify-center gap-4 mt-4">
