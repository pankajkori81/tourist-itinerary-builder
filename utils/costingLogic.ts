...
//   // Set to track IDs to prevent double counting if using "Ghost Stays" view logic
...
//       // Only process unique stays (avoids ghost stay double counting)
//       if (!processedStayIds.has(stay.id)) {
...
//     // 2. Calculate Activities (Only if 'included')
...
//         // Assuming Guide Fee is per group instance, usually. 
//         // If your logic is PP, multiply by travelers. Here I assume Guide Fee is Fixed Group Cost.
...
//     // 3. Calculate Transport (Only if 'Planned')
//     day.transports.forEach((trans) => {
//       if (trans.serviceType === 'Planned' && !processedTransportIds.has(trans.id)) {
//         transportCost += (trans.price || 0) * (trans.vehicleCount || 1);
//         processedTransportIds.add(trans.id);
//       }
//     });

//     // 4. Calculate Meals (Only if 'included')
//     day.meals?.forEach((meal) => {
//       if (meal.inclusionType === 'included' && !processedMealIds.has(meal.id)) {
//         // Simple logic: Assuming distinct adult/child logic is handled in input or simplified here
//         // For accurate calculation, we ideally need separate adult/child counts. 
//         // Using totalTravelers * adultCost for simplicity unless breakdown exists
